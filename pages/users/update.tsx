...
const UpdateProfile = () => {
  ...
  const [updateProfile, { data, loading }] =
    useMutation<UpdateResponse>(`/api/users/update`);
  ...
  useEffect(() => {
    ...
    if (user?.avatarId) {
      setPreview(
        `https://imagedelivery.net/NBXXy2GWfraNvFGhspc2LQ/${user.avatarId}/public`
      );
    }
  }, [user]);
  ...
  const onValid = async (data: UpdateProfileForm) => {
    ...
    if (avatars && avatars.length > 0) {
      const id = await uploadAvatar(avatars[0]);
    }
    await updateProfile({
  ...
  useEffect(() => {
    if (avatars && avatars.length > 0)
      setPreview(URL.createObjectURL(avatars[0]));
  }, [avatars]);
  ...
  return (
    <Layout title="Update Profile" before="/users/me">
      <form
        ...
        className="flex flex-col mt-16 px-5"
      >
        ...
        />
        {errorMessage && (
          <span className="mx-auto font-medium mt-2 text-sm text-orange-600">
            {errorMessage}
          </span>
        )}
